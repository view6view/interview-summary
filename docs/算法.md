...
```java
/**
 * @see <a href="https://leetcode.cn/problems/permutations/">leetcode</a>
 */
public List<List<Integer>> permute(int[] nums) {
    List<List<Integer>> res = new LinkedList<>();
    dfs(res, new LinkedList<>(), nums);
    return res;
}

private void dfs(List<List<Integer>> res, LinkedList<Integer> temp, int[] nums) {
    if (temp.size() == nums.length) {
        // 这里需要复制一个当前数组，最后temp会变成空
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/permutations-ii/">leetcode</a>
 */
public List<List<Integer>> permuteUnique(int[] nums) {
    List<List<Integer>> res = new LinkedList<>();
    // 排序（升序或者降序都可以），排序是剪枝的前提
    Arrays.sort(nums);
    dfsUnique(res, new LinkedList<>(), new boolean[nums.length],0, nums);
    return res;
}

private void dfsUnique(List<List<Integer>> res, LinkedList<Integer> temp,boolean[] used, int depth, int[] nums) {
    ...
    for (int i = 0; i < nums.length; i++) {
        ...
        temp.add(nums[i]);
        used[i] = true;
        dfsUnique(res, temp, used, depth + 1, nums);
        used[i] = false;
        temp.removeLast();
    }
}
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/palindrome-linked-list/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/">leetcode</a>
 */
public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
    ...
    // 如果pq节点不在左子树中，返回右子树的结果
    if (left == null) {
        return right;
    }
    // 如果pq节点不在右子树中，返回左子树的结果
    if (right == null) {
        return left;
    }
    return root;
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/product-of-array-except-self/">leetcode</a>
 */
public static int[] productExceptSelf(int[] nums) {
    int len = nums.length;
    // dp[i][0] 代表 i 左边的乘积，dp[i][1] 代表 i 右边的乘积
    int[][] dp = new int[len][2];
    dp[0][0] = 1;
    dp[len - 1][1] = 1;
    for (int i = 1, j = len - 2; i < len; i++, j--) {
        dp[i][0] = nums[i - 1] * dp[i - 1][0];
        dp[j][1] = nums[j + 1] * dp[j + 1][1];
    }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/search-a-2d-matrix-ii/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/move-zeroes/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/longest-increasing-subsequence/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/house-robber-iii/">leetcode</a>
 */
...
public int rob1(TreeNode root) {
    ...
    return robInner(root, map);
}

public int robInner(TreeNode root, Map<TreeNode, Integer> map) {
    if (root == null) return 0;
    ...
    int money = root.val;
    if (root.left != null) {
        money += (robInner(root.left.left, map) + robInner(root.left.right, map));
    }

    if (root.right != null) {
        money += (robInner(root.right.left, map) + robInner(root.right.right, map));
    }

    // 返回
    int max = Math.max(money, robInner(root.left, map) + robInner(root.right, map));
    map.put(root, max);
    return max;
}
```
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/top-k-frequent-elements/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/queue-reconstruction-by-height/">leetcode</a>
 */
public int[][] reconstructQueue(int[][] people) {
    ...
    for (int[] person : people) {
        if (list.size() > person[1]) {
            // 如果list的长度 大于 person 的前面应该有的人个数，插队
            list.add(person[1], person);
        } else {
            // 不大于 person前面应该有的人数时，直接将元素插入尾部
            list.add(person);
        }
    }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/find-all-anagrams-in-a-string/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/find-all-numbers-disappeared-in-an-array/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/hamming-distance/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/convert-bst-to-greater-tree/">leetcode</a>
 */
...
/**
 * @see <a href="https://leetcode.cn/problems/diameter-of-binary-tree/">leetcode</a>
 */
public int diameterOfBinaryTree(TreeNode root) {
    int maxHeight = getMaxHeight(root.left) + getMaxHeight(root.right);
    return Math.max(max, maxHeight);
}
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/subarray-sum-equals-k/">leetcode</a>
 */
public int subarraySum(int[] nums, int k) {
    ...
    for (int num : nums) {
        ...
        Integer val = preSumFreq.get(preSum - k);
        if (val != null) {
            count += val;
        }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/shortest-unsorted-continuous-subarray/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/merge-two-binary-trees/">leetcode</a>
 */
...
![image-20220523110832639](images/image-20220523110832639.png)

```java
/**
 * @see <a href="https://leetcode.cn/problems/task-scheduler/">leetcode</a>
 */
public int leastInterval(char[] tasks, int n) {
    ...
    return Math.max(tasks.length, (buckets[25] - 1) * (n + 1) + maxCount);
}
```
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/palindromic-substrings/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/ju-zhen-zhong-de-lu-jing-lcof/">leetcode</a>
 */
public boolean exist(char[][] board, String word) {
...
private boolean dfs(char[][] board, int row, int col, int idx, String word) {
    ...
    board[row][col] = '\n';
    int nextIdx = idx + 1;
    boolean res = dfs(board, row - 1, col, nextIdx, word)
            || dfs(board, row + 1, col, nextIdx, word)
            || dfs(board, row, col - 1, nextIdx, word)
            || dfs(board, row, col + 1, nextIdx, word);
    board[row][col] = word.charAt(idx);
    return res;
}
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/ji-qi-ren-de-yun-dong-fan-wei-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/jian-sheng-zi-ii-lcof/">leetcode</a>
 */
public static int cuttingRope(int n) {
    if (n < 4) {
        return n - 1;
    }
    return (int) cuttingRopeInner(n);
}

private static long cuttingRopeInner(int n) {
    if (n <= 4) {
        return n;
    } else {
        return  3 * cuttingRopeInner(n - 3) % 1_000_000_007;
    }
}
```
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/shu-zhi-de-zheng-shu-ci-fang-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/biao-shi-shu-zhi-de-zi-fu-chuan-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/zhan-de-ya-ru-dan-chu-xu-lie-lcof/">leetcode</a>
 */
public boolean validateStackSequences(int[] pushed, int[] popped) {
    int len = pushed.length;
    int[] stack = new int[len];
    // 栈指针、入栈数组指针，出栈数组指针
    int sIdx = -1, inIdx = 0, outIdx = 0;
    while (inIdx < len) {
        if (pushed[inIdx] == popped[outIdx]) {
            outIdx++;
            while (sIdx > -1 && stack[sIdx] == popped[outIdx]) {
                // 元素出栈
                sIdx--;
                // 出栈数组指针后移
                outIdx++;
            ...
            sIdx++;
            stack[sIdx] = pushed[inIdx];
        }
        inIdx++;
    }
    while (outIdx < len) {
        if (stack[sIdx] == popped[outIdx]) {
            sIdx--;
            outIdx++;
        } else {
            return false;
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/cong-shang-dao-xia-da-yin-er-cha-shu-lcof/">leetcode</a>
 */
...
/**
 * 开始后进<br>
 * 第一层 前出（lr），后进（先l，后r）<br>
 * 第二层 后出（rl），前进（先r，后l）<br>
 * 第三层 前出（lr），后进（先l，后r）<br>
 * @see <a href="">leetcode</a>
 */
public List<List<Integer>> levelOrder(TreeNode root) {
    ...
    int size = 1;
    while (!deque.isEmpty()) {
        size = packageLevel(deque, lf, size, res);
        lf = !lf;
    }
    return res;
}

private int packageLevel(Deque<TreeNode> deque, boolean lr, int size, List<List<Integer>> res) {
    List<Integer> list = new ArrayList<>(size);
    if (lr) {
        for (int i = 0; i < size; i++) {
            TreeNode treeNode = deque.pollFirst();
            list.add(i, treeNode.val);
            if (treeNode.left != null) {
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/er-cha-sou-suo-shu-de-hou-xu-bian-li-xu-lie-lcof/">leetcode</a>
 */
...
public List<List<Integer>> pathSum(TreeNode root, int target) {
    ...
    dfs(root, 0, list, res, target);
    return res;
}

private void dfs(TreeNode treeNode, int preTotal, List<Integer> preList, List<List<Integer>> res, int target) {
    preTotal += treeNode.val;
    ...
    if (treeNode.left == null && treeNode.right == null) {
        ...
        if (treeNode.left != null) {
            dfs(treeNode.left, preTotal, preList, res, target);
        }
        if (treeNode.right != null) {
            dfs(treeNode.right, preTotal, preList, res, target);
        }
    }
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/fu-za-lian-biao-de-fu-zhi-lcof/">leetcode</a>
 */
public Node copyRandomList(Node head) {
    ...
    while (node.next != null) {
        if (node.random != null) {
            list.get(idx).random = list.get(map.get(node.random));
        }
        list.get(idx).next = list.get(idx + 1);
        node = node.next;
        idx++;
    }
    if (node.random != null) {
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/zi-fu-chuan-de-pai-lie-lcof/">leetcode</a>
 */
public String[] permutation(String s) {
    char[] chars = s.toCharArray();
    List<String> res = new LinkedList<>();
    dfs(res, 0, chars.length - 1, chars);
    return res.toArray(new String[res.size()]);
}
private void dfs(List<String> res, int idx, int end, char[] chars) {
    if (idx == end) {
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/ba-shu-zu-pai-cheng-zui-xiao-de-shu-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/shu-zi-xu-lie-zhong-mou-yi-wei-de-shu-zi-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/ba-shu-zi-fan-yi-cheng-zi-fu-chuan-lcof/">leetcode</a>
 */
public int translateNum(int num) {
    ...
    for (int i = 2; i <= len; i++) {
        Integer cur = Integer.valueOf(str.substring(i - 2, i));
        if (cur > 9 && cur < 26) {
            if (isolation) {
                dp[i] = dp[i - 1] * 2;
                isolation = false;
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/li-wu-de-zui-da-jie-zhi-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/shu-zu-zhong-shu-zi-chu-xian-de-ci-shu-lcof/submissions/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/chou-shu-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/he-wei-sde-lian-xu-zheng-shu-xu-lie-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/dui-lie-de-zui-da-zhi-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/nge-tou-zi-de-dian-shu-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/yuan-quan-zhong-zui-hou-sheng-xia-de-shu-zi-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/gu-piao-de-zui-da-li-run-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/qiu-12n-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/bu-yong-jia-jian-cheng-chu-zuo-jia-fa-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/gou-jian-cheng-ji-shu-zu-lcof/">leetcode</a>
 */
...
```java
/**
 * @see <a href="https://leetcode.cn/problems/ba-zi-fu-chuan-zhuan-huan-cheng-zheng-shu-lcof/">leetcode</a>
 */
public int strToInt(String str) {
    int res = 0, bndry = Integer.MAX_VALUE / 10;
    int idx = 0, sign = 1, length = str.length();
...
![image-20220427164222294](images/image-20220427164222294.png)
...
![image-20220427164700590](images/image-20220427164700590.png)

**输入描述:**

![image-20220427164726717](images/image-20220427164726717.png)
...
> ![img](images/B284A414209914402674986D66CCCF84.png)
>
> 3个节点为3，2，5
> 节点3与2的最短距离为3
> 节点3与5的最短距离为3
...
![image-20220427165111947](images/image-20220427165111947.png)

**输入描述:**

![image-20220427165143450](images/image-20220427165143450.png)
